...
const PG100005: React.FC<PG100005Props> = ({ documentCode }) => {
    ...
    const displayData =
        currentLang === "KO" ? data : translations[currentLang]?.data;
    const displaySummary =
        currentLang === "KO" ? summary : translations[currentLang]?.summary;
    ...
    const handleExportPDF = async () => {
        ...
        a.href = url;
        a.download =
            i18n.language === "KO" ? "분석보고서.pdf" : "AnalysisReport.pdf";
        a.click();
    ...
    const handleTranslate = React.useCallback(
        async (targetLang: "EN" | "JP" | "ZH") => {
            // 번역본이 없으면 API 호출
            if (!data || !summary) return;
            setIsTranslating(true);
            try {
                // Backend는 소문자 ISO 코드(en/ja/zh)를 기대하므로 매핑합니다.
                const backendLangMap: Record<
                    "EN" | "JP" | "ZH",
                    "en" | "ja" | "zh"
                > = {
                    EN: "en",
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                console.log("PG100005: API 응답 받음", {
                ...
                if (!cancelled) {
                    ...
                    setSummary({
                        analysisReport: {
                            summary:
                                summaryRes.data.analysisReport?.summary ?? "",
                            riskLevel:
                                summaryRes.data.analysisReport?.riskLevel ?? "",
                            sentimentSummary:
                                summaryRes.data.analysisReport
                                    ?.sentimentSummary ?? "",
                            sentimentScore:
                                summaryRes.data.analysisReport
                                    ?.sentimentScore ?? 0,
                        ...
                        riskyClauses: {
                            clauseSummary:
                                summaryRes.data.riskyClauses?.clauseSummary ??
                                "",
                            legalRisk:
                                summaryRes.data.riskyClauses?.legalRisk ?? "",
                            financialImpact:
                                summaryRes.data.riskyClauses?.financialImpact ??
                                "",
                            operationalImpact:
                                summaryRes.data.riskyClauses
                                    ?.operationalImpact ?? "",
                            recommendedAction:
                                summaryRes.data.riskyClauses
                                    ?.recommendedAction ?? "",
                        ...
                        transactionAnomaly: {
                            price:
                                summaryRes.data.transactionAnomaly?.price ?? 0,
                            averagePrice:
                                summaryRes.data.transactionAnomaly
                                    ?.averagePrice ?? 0,
                            deviationPercent:
                                summaryRes.data.transactionAnomaly
                                    ?.deviationPercent ?? 0,
                            isAnomaly:
                                summaryRes.data.transactionAnomaly?.isAnomaly ??
                                false,
                        },
                    });
        ...
        return () => {
            console.log(
                "PG100005: 컴포넌트 unmount 또는 재실행으로 인한 클린업"
            );
            cancelled = true;
        };
    }, [documentCode, i18n]);
    ...
    const restEntries = useMemo(() => {
        ...
        const keys = Object.keys(
            displayData
        ) as (keyof StructuredContractDataDTO)[];
        ...
        return keys
            ...
            .map((k) => {
                ...
                const label =
                    currentLang !== "KO"
                        ? t(k, { ns: "contractFields" })
                        : contractFieldLabels[k] ?? (k as string);
        ...
        return (
            <div className="rv05-container">
                <CommonContainerHeader
                    ...
                    description={t("analysis_report_title")}
                />
                {/* 스켈레톤 UI */}
                <div
                    className="rv05-banner skeleton"
                ...
                <section className="rv05-grid">
                    <article className="rv05-card">
                        ...
                        <div className="skeleton skeleton-hr"></div>
                        <div
                            className="skeleton-dl"
                            style={{ gridTemplateColumns: "1fr" }}
                        >
                            {[...Array(6)].map((_, i) => (
                                <div
                                    key={i}
                                    className="skeleton skeleton-dd"
                                    style={{
                                        width: "100%",
                                        marginBottom: "8px",
                                    }}
    ...
    if (!data) {
        // 데이터가 아예 없는 경우 (이론상 발생하기 어려움)
        return (
            <div className="rv05-container">
                <div className="rv05-card">
                    데이터가 아직 준비되지 않았습니다.
                </div>
            </div>
        );
            ...
            <div className="rv05-container">
                ...
                {loading ? (
                    ...
                    <div className="rv05-banner" style={bannerStyle}>
                        <div>
                            <div className="rv05-banner-title">
                                {t("comprehensive_risk_assessment")}
                            </div>
                            <div className="rv05-banner-sub">
                                <SkeletonWrapper width="90%">
                                    {displaySummary?.analysisReport?.summary ??
                                        "-"}
                                </SkeletonWrapper>
                            </div>
                            <br />
                        ...
                        <div
                            className={`rv05-badge ${(() => {
                                const level =
                                    displaySummary?.analysisReport?.riskLevel?.toLowerCase();
                                switch (level) {
                                    case "high":
                                    case "critical":
                                    case "고위험":
                ...
                {loading ? (
                    ...
                    <article className="rv05-card rv05-wide transaction-anomaly">
                        <h3 className="rv05-sec-title">
                            {t("transaction_anomaly_detection")}
                        </h3>
                        ...
                        <dl className="rv05-dl">
                            <div>
                                ...
                                <dd>
                                    <SkeletonWrapper>
                                        {fmtNum(
                                            displaySummary?.transactionAnomaly
                                                ?.price
                                        )}{" "}
                                        {t("currency_unit")}
                                    </SkeletonWrapper>
                    ...
                    <article className="rv05-card">
                        ...
                        <dl className="rv05-dl">
                            ...
                            <div>
                                ...
                                <dd>
                                    <SkeletonWrapper>
                                        {fmtDate(displayData?.leasePeriodStart)}{" "}
                                        ~ {fmtDate(displayData?.leasePeriodEnd)}
                                    </SkeletonWrapper>
                                </dd>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <dd>
                                    <SkeletonWrapper>
                                        {fmtDate(displayData?.balanceDate)}
                                    </SkeletonWrapper>
                                </dd>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <dd>
                                    <SkeletonWrapper>
                                        {fmtNum(displayData?.landArea)} /{" "}
                                        {fmtNum(displayData?.buildingArea)} /{" "}
                                        {fmtNum(displayData?.leaseArea)} ㎡
                                    </SkeletonWrapper>
                                </dd>
                            </div>
                        </dl>
                    ...
                    <article className="rv05-card">
                        ...
                        <dl className="rv05-dl">
                            ...
                            <div>
                                ...
                                <dd>
                                    <SkeletonWrapper>
                                        {fmtNum(displayData?.downPayment)}{" "}
                                        {t("currency_unit")}
                                    </SkeletonWrapper>
                                </dd>
                            </div>
                            <div>
                                ...
                                <dd>
                                    <SkeletonWrapper>
                                        {fmtNum(displayData?.middlePayment)}{" "}
                                        {t("currency_unit")}
                                    </SkeletonWrapper>
                                </dd>
                            </div>
                            ...
                            <div>
                                ...
                                <dd>
                                    <SkeletonWrapper>
                                        {fmtNum(displayData?.rentAmount)}{" "}
                                        {t("currency_unit")} (
                                        {t(displayData?.rentType ?? "-")})
                                    </SkeletonWrapper>
                                </dd>
                            </div>
                        </dl>
                ...
                <div className="rv05-actions">
                    {currentLang !== "KO" ? (
                        ...
                        <div
                            className={`rv05-translate-dropdown ${
                                isTranslateDropdownOpen ? "is-open" : ""
                            }`}
                        >
                            <button
                                className="an02-ai-analyze-start-btn"
