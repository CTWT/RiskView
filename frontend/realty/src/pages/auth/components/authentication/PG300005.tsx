 ...
 */
const PG300005: React.FC<PG300005Props> = ({
    onNext,
    onBackToEmail,
    userEmail,
}) => {
    ...
    const handleVerifyCode = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(error)) {
                showToast(error.response?.data || "이메일 인증 실패", {
                    type: "error",
                });
            } else {
    ...
    const handleResendCode = async () => {
        ...
        try {
            ...
            // 응답에서 메시지 추출
            const { message } = response.data;
            showToast(message || "인증 메일이 다시 전송되었습니다.", {
                type: "success",
            });
        } catch (error) {
            if (axios.isAxiosError(error)) {
                showToast(
                    error.response?.data?.message || "인증 메일 재전송 실패",
                    { type: "error" }
                );
            } else {
