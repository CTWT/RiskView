...
const PG500042: React.FC = () => {
    ...
    //                 : [],
    //             __fromWrite: Boolean(
    //                 (location.state as PostDetailWithFlags)?.__fromWrite ??
    //                     false
    //             ),
    //         };
    //     },
    ...
    const normalize = useCallback(
        (data: ApiPost): PostDetailWithFlags => {
            const normalizedPost: PostDetailWithFlags = {
                id: Number(data.id ?? 0),
                board: data.board ?? "free",
                title: String(data.title ?? ""),
                author: String(data.author ?? "익명"),
                ...
                likedByMe: Boolean(data.likedByMe ?? false),
                comments: Array.isArray(data.comments)
                    ? data.comments.map((c: ApiComment) => {
    ...
    const fetchCurrentUser = useCallback(async () => {
        ...
        // console.log("사용자는 : ", currentUser);
    }, []); // 의존성 없음 → stable function
    ...
    React.useEffect(() => {
        const st: PostDetailWithFlags | undefined =
    ...
    const deletePostNow = async () => {
        ...
        try {
            ...
            // 4) 성공: 목록으로 이동
            console.log(
                `[PG500042] 게시글 ID ${id} 삭제 성공. 목록으로 이동합니다.`
            );
    ...
    const handleLike = async () => {
        ...
        console.log(
            `[PG500042] 게시글 ID ${pid} 좋아요 토글. 이전 상태: ${prevLiked}`
        );
        ...
        try {
            ...
            const res = await fetch(`/api/posts/${pid}/likes`, {
                method,
                headers: { "Content-Type": "application/json" },
            });
            if (res.ok) {
    ...
    const handleCommentSubmit = async () => {
        ...
        // 1) 인증 검사: 비로그인 시 로그인 페이지로 이동
        if (!isAuthed()) {
            showToast("댓글 작성은 로그인 후 이용 가능합니다.", {
                type: "error",
            });
            navigate("/login", {
                state: { from: location.pathname + location.search },
    ...
    return (
        ...
        <PageContainer showBreadcrumb={true} centerContent={true}>
            <div className="post-detail">
                ...
                {/* 게시글 로딩 중 표시 */}
                {isLoading && (
                    <div className="post-loading">게시글을 불러오는 중…</div>
                )}
                    ...
                    <div className="post-editing" style={{ marginTop: 12 }}>
                        ...
                        >
                            ...
                            <input
                                ...
                                value={postEditTitle}
                                onChange={(error) =>
                                    setPostEditTitle(error.target.value)
                                }
                        ...
                        <div className="form-section">
                            ...
                            <textarea
                                ...
                                onChange={(error) =>
                                    setPostEditContent(error.target.value)
                                }
                ...
                <section className="comments-section">
                    ...
                    <div className="comment-form">
                        <textarea
                            value={newComment}
                            onChange={(error) =>
                                setNewComment(error.target.value)
                            }
                            onKeyDown={(error) => {
                                if (
                ...
                <div className="post-navigation">
                    <Link
                        to={`/PG500001/PG500041?board=${post?.board || "free"}`}
