...
const PG500043: React.FC = () => {
    ...
    const initialBoard: BoardKey =
        ...
        (location.state as PostDetailWithFlags)?.board ||
        "free";
    ...
    logInfo("컴포넌트.초기화", {
        isEditMode: Boolean(editPost?.id),
        postId: editPost?.id,
    });
    ...
    const handleImageUpload = (fileName: string) => {
        uploadedImagesRef.current = [...uploadedImagesRef.current, fileName];
    };
    ...
    const handleSubmitPost = async () => {
        ...
        try {
            ...
            if (isEditMode && editPost?.id) {
                ...
                try {
                    ...
                    const idx = posts.findIndex(
                        (p: PostDetailWithFlags) => p.id === editPost.id
                    );
                    if (idx >= 0) {
                        posts[idx] = {
                            ...posts[idx],
                    ...
                    navigate(
                        `/PG500001/PG500041/PG500042/${editPost.id}?board=${initialBoard}`,
                        { replace: true }
                    );
                    return;
                }
            ...
            try {
                ...
                navigate(
                    `/PG500001/PG500041/PG500042/${data.id}?board=${initialBoard}`,
                    { replace: true }
                );
                return;
            } catch (error) {
                ...
                navigate(
                    `/PG500001/PG500041/PG500042/${newPostData.id}?board=${initialBoard}`,
                    {
                        replace: true,
