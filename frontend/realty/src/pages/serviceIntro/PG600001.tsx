      ...
      {features.map((feature) => (
        <button
          ...
          className={`in01-feature-button ${
            selectedFeature === feature.id ? "in01-feature-button-active" : ""
          }`}
        ...
        >
          ...
          <div className="in01-feature-icon-wrapper">
            {feature.icon && (
              <div className="in01-feature-icon">{feature.icon}</div>
            )}
          </div>
          <div className="in01-feature-text-wrapper">
            <span className="in01-feature-title">{feature.title}</span>
            {/* ⭐ 부제목 렌더링 로직 추가 ⭐ */}
            {feature.subtitle && (
              <span className="in01-feature-subtitle">{feature.subtitle}</span>
            )}
          </div>
        </button>
      ))}
  ...
  return (
    <div className="in01-container">
      ...
      <div className="an02-header">
        <h2 className="an02-subtitle">핵심 기능</h2>
        <h1 className="an02-title">숨겨진 리스크를 한눈에 파악하세요</h1>
        <p className="an02-description">
          RiskView는 AI 기술을 활용하여 부동산 계약서와 등기부등본을 분석하고
          숨겨진 위험 요소를 찾아냅니다.
        </p>
      </div>
